...
if page == translations[language]["home"]:
    st.markdown("""
        <style>
        /* Style for all select boxes and radio buttons */
        .stSelectbox, .stRadio {
    ...
    work_options = {
        ...
        translations[language]["govt_job"]: "🏛️ Government Employee",
    ...
    weight = st.number_input(f"⚖️ {translations[language]['weight']}", min_value=1.0, max_value=200.0, value=70.0)
    height = st.number_input(f"📏 {translations[language]['height']}", min_value=50.0, max_value=250.0, value=170.0)

    bmi = weight / ((height / 100) ** 2)
    st.markdown(f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 10px; margin: 10px 0;'><h4>📊 {translations[language]['bmi']}: {bmi:.2f}</h4></div>", unsafe_allow_html=True)

    # Smoking status with icons
    smoking_options = {
        translations[language]["never_smoked"]: "🚭 Never Smoked",
        translations[language]["formerly_smoked"]: "🚬➡️❌ Formerly Smoked",
    ...
    if st.button(translations[language]["predict_button"]):
        try:
            ...
            if probability is not None:
                ...
                if heart_disease:
                    st.markdown("<div class='risk-factor'>❤️ Heart Disease</div>", unsafe_allow_html=True)
                if bmi > 30:
                    st.markdown("<div class='risk-factor'>⚖️ High BMI</div>", unsafe_allow_html=True)
                if glucose > 140:
